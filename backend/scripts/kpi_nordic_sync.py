#!/usr/bin/env python3
"""
KPI Nordic Data Synchronization Script
Fetches Nordic KPI statistics from Börsdata API and populates psw_marketdata.kpi_nordic table

API Endpoint: https://apiservice.borsdata.se/v1/instruments/kpis/{kpiId}/{group}/{calculation}
Target Database: psw_marketdata
Target Table: kpi_nordic

This script fetches Nordic-specific Key Performance Indicator statistics from Börsdata's API
for portfolio analysis and benchmarking against Nordic market data.
...
API_ENDPOINT_TEMPLATE = '/v1/instruments/kpis/{kpi_id}/{group}/{calculation}'
BORSDATA_AUTH_KEY = os.getenv('BORSDATA_AUTH_KEY', '55d57eb27768456b9aa975e158d12898')

API_HEADERS = {
    'accept': 'text/plain',
    'User-Agent': 'PSW-KPI-Nordic-Sync/1.0'
}

# Configuration for Nordic KPI data to fetch
...
KPI_FETCH_CONFIG = [
    ...
    (2, '1year', 'median'),  # P/E 1-year median
    (1, '1year', 'median'),  # Dividend Yield 1-year median
...
LOG_FILENAME = os.path.join(LOG_DIR, "kpi_nordic_sync.log")
...
class KPINordicSync:
    """Handles synchronization of Nordic KPI data from Börsdata API to database"""
    ...
    def fetch_kpi_nordic_data(self, kpi_id: int, group: str, calculation: str) -> Optional[Dict[str, Any]]:
        """Fetch Nordic KPI data for specific parameters from Börsdata API"""
        try:
            # Build URL
            ...
            logger.debug(f"Fetching Nordic KPI data: KPI={kpi_id}, Group={group}, Calc={calculation}")
            ...
            if response.status_code == 200:
                data = response.json()
                logger.debug(f"Successfully fetched Nordic data for KPI {kpi_id}/{group}/{calculation}: {len(data.get('values', []))} values")
                return data
                
            elif response.status_code == 404:
                logger.warning(f"No Nordic data found for KPI {kpi_id}/{group}/{calculation} (404)")
                return None
            ...
            logger.error(f"API request timed out for Nordic KPI {kpi_id}/{group}/{calculation}")
            return None
        except requests.exceptions.ConnectionError:
            logger.error(f"Failed to connect to API endpoint for Nordic KPI {kpi_id}/{group}/{calculation}")
            return None
        except requests.exceptions.RequestException as e:
            logger.error(f"API request failed for Nordic KPI {kpi_id}/{group}/{calculation}: {e}")
            return None
        except json.JSONDecodeError as e:
            logger.error(f"Failed to parse API response as JSON for Nordic KPI {kpi_id}/{group}/{calculation}: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error during API fetch for Nordic KPI {kpi_id}/{group}/{calculation}: {e}")
            return None
    
    def process_kpi_values(self, kpi_data: Dict[str, Any], kpi_id: int, group: str, calculation: str) -> int:
        """Process and insert/update Nordic KPI values from API response"""
        processed_count = 0
        
        try:
            values = kpi_data.get('values', [])
            if not values:
                logger.warning(f"No values found in response for Nordic KPI {kpi_id}/{group}/{calculation}")
                return 0
            
            # Prepare SQL statement with ON DUPLICATE KEY UPDATE
            insert_sql = """
                INSERT INTO kpi_nordic (kpi_id, group_period, calculation, instrument_id, numeric_value, string_value)
                VALUES (%s, %s, %s, %s, %s, %s)
                ...
                try:
                    ...
                    if numeric_value is not None:
                        try:
                            ...
                            logger.warning(f"Invalid numeric value for Nordic instrument {instrument_id}: {numeric_value}")
                            numeric_value = None
                    ...
                    logger.error(f"Error processing Nordic value item {value_item}: {e}")
                    self.stats['errors'] += 1
                    continue
            ...
            if batch_data:
                ...
                logger.debug(f"Processed {len(batch_data)} Nordic values for KPI {kpi_id}/{group}/{calculation}")
            
            return processed_count
            
        except mysql.connector.Error as err:
            logger.error(f"Database error processing Nordic KPI values {kpi_id}/{group}/{calculation}: {err}")
            self.stats['errors'] += 1
            return 0
        except Exception as e:
            logger.error(f"Unexpected error processing Nordic KPI values {kpi_id}/{group}/{calculation}: {e}")
            self.stats['errors'] += 1
            return 0
    ...
    def sync_kpi_nordic_data(self) -> bool:
        """Main synchronization process"""
        try:
            logger.info("Starting KPI Nordic data synchronization...")
            ...
            logger.info(f"Processing {total_configs} Nordic KPI configurations...")
            
            for i, (kpi_id, group, calculation) in enumerate(KPI_FETCH_CONFIG, 1):
                logger.info(f"Processing Nordic configuration {i}/{total_configs}: KPI {kpi_id}/{group}/{calculation}")
                ...
                # Fetch data from API
                kpi_data = self.fetch_kpi_nordic_data(kpi_id, group, calculation)
                ...
                if kpi_data:
                    ...
                    try:
                        ...
                        logger.debug(f"Committed {processed} Nordic records for KPI {kpi_id}/{group}/{calculation}")
                    except mysql.connector.Error as err:
                        logger.error(f"Failed to commit Nordic KPI {kpi_id}/{group}/{calculation}: {err}")
                        self.db_connection.rollback()
                        self.stats['errors'] += 1
                else:
                    logger.warning(f"No Nordic data received for KPI {kpi_id}/{group}/{calculation}")
                    self.stats['skipped'] += 1
            ...
            logger.info("KPI Nordic data synchronization completed")
            ...
            logger.error(f"Unexpected error during Nordic synchronization: {e}")
...
def main():
    ...
    print("=" * 60)
    print("KPI NORDIC DATA SYNCHRONIZATION SCRIPT STARTED")
    print("=" * 60)
    
    # File logging with timestamps
    logger.info("=" * 60)
    logger.info("KPI NORDIC DATA SYNCHRONIZATION SCRIPT STARTED")
    logger.info("=" * 60)
    
    try:
        # Create synchronizer instance and run
        sync = KPINordicSync()
        success = sync.sync_kpi_nordic_data()
